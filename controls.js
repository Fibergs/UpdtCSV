...
function updateDashboard() {
    ...
    animateValue('carbonKgValue', results.totals.carbon_avoided, 1, '');

    // Update cost breakdown sub-rows
    updateCostBreakdown(results.totals.bins_cost_chg, results.totals.gloves_cost_chg);

    // Update carbon donut segments
    updateCarbonDonut(results.totals.carbon_by_category);

    // Update triple metrics with animated counting
    animateValue('weightReduced', results.totals.kg_saved, 1, '');
    animateValue('carbonAvoided', results.totals.carbon_avoided, 1, '');
    ...
    updateCategoryBreakdown('cell_culture', results.categories.cell_culture);
...
/**
 * Update a category breakdown display (Part 15.2 — sign-bug fix + safe clamps)
 */
function updateCategoryBreakdown(category, data) {
    ...
    if (changeEl) {
        // change is always a reduction % (0–100). Clamp to 0 as safety floor.
        const changePercent = Math.max(0, data.change);
        changeEl.textContent = `-${changePercent.toFixed(1)}%`;
        changeEl.style.color = changePercent > 0 ? 'var(--color-accent)' : 'var(--text-secondary)';
    }

    if (barEl && data.pre > 0) {
        // Bar shows remaining fraction — shrinks as reduction grows
        const remainingPct = Math.max(0, Math.min(100, (data.post / data.pre) * 100));
        barEl.style.width = `${remainingPct}%`;
    }
}

/**
 * Update cost breakdown sub-row pills with component savings/spend
 * positive value = cost saving, negative = cost increase
 */
function updateCostBreakdown(binsChg, glovesChg) {
    function setPill(id, value) {
        const el = document.getElementById(id);
        if (!el) return;
        el.className = 'cost-split-pill';
        if (Math.abs(value) < 0.5) {
            el.classList.add('cost-pill-neutral');
            el.textContent = 'no change';
        } else if (value > 0) {
            el.classList.add('cost-pill-save');
            el.textContent = `saves ${value.toFixed(1)}%`;
        } else {
            el.classList.add('cost-pill-spend');
            el.textContent = `+${Math.abs(value).toFixed(1)}% cost`;
        }
    }
    setPill('costGlovesPill', glovesChg);
    setPill('costBinsPill', binsChg);
}

/**
 * Update the carbon donut chart segments from per-category CO₂ avoided (kg CO₂e)
 * Each segment arc is proportional to that category's share of total carbon avoided.
 * The filled arc combined equals carbon_reduction_percent of the circumference.
 */
function updateCarbonDonut(carbonByCategory) {
    const r = 70;
    const C = 2 * Math.PI * r; // 439.82
    const GAP = 5;              // visual gap between adjacent segments

    const cats = [
        { key: 'bins',         id: 'carbonSegBins' },
        { key: 'gloves',       id: 'carbonSegGloves' },
        { key: 'pipettes',     id: 'carbonSegPipettes' },
        { key: 'tubes',        id: 'carbonSegTubes' },
        { key: 'cell_culture', id: 'carbonSegCulture' }
    ];

    const total = cats.reduce((s, c) => s + Math.max(0, (carbonByCategory || {})[c.key] || 0), 0);

    if (total <= 0) {
        cats.forEach(c => {
            const el = document.getElementById(c.id);
            if (el) { el.style.strokeDasharray = `0 ${C}`; el.style.strokeDashoffset = 0; }
        });
        return;
    }
    ...
    cats.forEach(c => {
        const el = document.getElementById(c.id);
        if (!el) return;
        const proportion = Math.max(0, (carbonByCategory[c.key] || 0)) / total;
        const fullArc   = proportion * C;
        const visArc    = Math.max(0, fullArc - GAP);
        el.style.strokeDasharray  = `${visArc.toFixed(2)} ${(C - visArc).toFixed(2)}`;
        el.style.strokeDashoffset = (-cumStart).toFixed(2);
        cumStart += fullArc;
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Link bins_reduce ↔ bins_woodsafe: dragging one always moves the other
    const woodsafeFader = faderControls.find(f => f.controlKey === 'bins_woodsafe');
    const reduceFader   = faderControls.find(f => f.controlKey === 'bins_reduce');
    if (woodsafeFader && reduceFader) {
        woodsafeFader.input.addEventListener('input', () => {
            reduceFader.setValue(woodsafeFader.value);
            calculator.setInput('bins_reduce', woodsafeFader.value);
            updateDashboard();
        });
        reduceFader.input.addEventListener('input', () => {
            woodsafeFader.setValue(reduceFader.value);
            calculator.setInput('bins_woodsafe', reduceFader.value);
            updateDashboard();
        });
    }
